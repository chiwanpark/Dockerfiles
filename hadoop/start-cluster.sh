...
docker run -itd --env DAEMON="master" --volume="$MASTER_VOLUME:/root" --volume="${VOLUMES[0]}:/hadoop-data" --net="$NETWORK" --name hadoop-master --hostname hadoop-master chiwanpark/hadoop:$VERSION
